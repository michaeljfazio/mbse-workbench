...
| 14 | Round-trip integrity | 1 | Project → JSON → project is lossless. Project → SysML v2 text → project is structurally identical modulo IDs. The FBW model survives both. | walk-20 (JSON ✓ / SysML broken by #446 — serializer emits unquoted multi-word ids; advances to 3 once #446 lands + re-run walk passes) |
