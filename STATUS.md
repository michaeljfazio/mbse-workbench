...
## Current iteration
- Iteration #: 897
- Started: 2026-05-19 (UTC, post-#570 merge at `25bb92e`)
- Branch: `phase-15/iter-897-status-sync` (off main `25bb92e`) — STATUS sync chore PR
- Working on: STATUS sync for the iter-897 second dim-17 batch (#564 + #566 shipping via PR #571)

## Last test run
- Engineer subagent ran `pnpm run check` after initial implementation — typecheck PASS, build had a latent `tsc -b` error in the new `src/viewpoints/shared/edgePath.ts` (return type tuple mismatch with React Flow v12's actual 5-element path tuple, missed because `tsc --noEmit` ran loosely); fixed in the fix-up commit `9c40b2d`.
- Fix-up subagent re-ran `pnpm run check` after addressing the 3 IMPORTANTs: **typecheck PASS, lint PASS (3 pre-existing warnings, 0 errors), unit 1524/1524 PASS, build PASS, e2e 24/24 PASS on chromium + webkit including the 2 new visual baselines**.
- CI on PR #571 IN_PROGRESS as STATUS.md is being written; auto-merge SQUASH enabled.
...
## Last PR sweep
- Iter-897 launch: `gh pr list --state open` returned `[#570 (BLOCKED + MERGEABLE, CI IN_PROGRESS, auto-merge SQUASH enabled)]` — left per AGENT.md "CI IN_PROGRESS → leave it." #570 auto-merged to `25bb92e` during iter-897 execution. This iteration opened one feature PR: #571 (`feat(canvas,inspector): per-edge routing-style and stroke-style/color pickers`, closes #564 + #566, auto-merge SQUASH pending). **In-flight 1 / 5 of A.8 cap** at iter-897 close (#571 only).

## Known issues / blockers
- None. The 2 remaining dim-17 issues (#563/#565) are the *expected* iter-898+ engineer-batch backlog (third dim-17 batch grouping waypoint editing + label drag on the shared per-edge `data` extension introduced by #571). The pre-existing webkit a11y color-contrast flake on the toolbar undo button persists across iters and is not caused by this iteration's changes; it is its own dim-25 finding and will be addressed when dim-25 is the active focus.

## Open phase:15 issues at iter-897 close (after #571 merges)
...
**Iter-898 — engineer hat → dispatch the third (and final) dim-17 engineer batch grouping #563 (PC2/PC3 waypoint editing) + #565 (PC5 label drag) on `phase-15/dim17-edge-waypoints-and-label-drag`.** Both features extend the per-edge `data` infrastructure landed by #571 (iter-897) — waypoint editing adds `waypoints: { x: number; y: number }[]` to `EdgeBase` + element-as-edge types; label drag adds `labelOffset: { dx: number; dy: number }` (or equivalent). Inspector + canvas surface controls (right-click context menu for insert/remove waypoint per #563 acceptance criteria; drag-handle on the edge label per #565). Per A.8 soft cap (5), iter-898's dispatch is permitted in parallel with #571's CI tail if the latter hasn't yet auto-merged.

**Iter-898 process discipline (reinforced from iter-897):** Pre-PR review subagent dispatched BEFORE PR open (not just before auto-merge enable) — the strict serialisation is: engineer subagent pushes branch → code-review subagent reports → fix-ups applied → THEN `gh pr create` → THEN `gh pr merge --auto --squash`. Iter-897 validated this protocol: 3 IMPORTANTs caught by review were absorbed in the same PR as a single fix-up commit, avoiding the iter-896 stacked-PR pattern.

**Iter-898 process discipline reminder (carried from iter-896):** Per A.5, the main agent's role is reconciliation + review. Engineer batches go to Sonnet subagents in isolated worktrees; code review goes to a separate Sonnet subagent reading the diff. The main agent does NOT implement features — it briefs subagents, reviews their work, opens PRs.

**Iter-898 setup for #563 + #565:** the engineer subagent should be briefed to study iter-897's `src/viewpoints/shared/edgePath.ts` helper as the integration point for waypoint-aware path computation, and the iter-897 Inspector edge-style controls as the styling template for label-drag UX. React Flow v12 native waypoint support uses the `<BaseEdge>` `path` prop directly — the helper should compute a piecewise path through the waypoint list when present.

**Halting safety:** STOP file / `status:emergency-stop` label unchanged; Phase-15 iter-count at 105, well under the 300 churn ceiling.

**In-flight at iter-897 close (1 / 5 of A.8 cap until PR #571 merges, then 0 / 5):**
- `phase-15/dim17-inspector-edge-style-pickers` (PR #571) — auto-merge SQUASH pending CI.
