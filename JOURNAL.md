...
## Iteration 889 — 2026-05-19 — walk-34 surfaced #548; convergence chain reset 1 → 0 / 3; dim-10 score-3 promotion held

**Event:** escalation

**Phase:** phase:15 — Architect-driven UX & feature hardening

**Narrative:** Walk-34 was supposed to be the chain[1] retry that walk-33 set up: a regression broad-sweep against the released `vphase-15.10` / `v1.6.1` bundle (first agent-cut release under ADR 0017), validating that PR #531's added `Position.Right` and `Position.Bottom` source handles on `ActorNode` closed the bidirectional Actor↔UseCase association gap. The primary direction (`usecase.right → actor.left`) passed cleanly as expected; the secondary direction (`actor.right → usecase.left`) did not. The driver located both endpoints in the DOM under the new handle declarations (`react-flow__handle-right` on `[data-testid="use-case-actor-<id>"]` was definitely present — the iter-879 fix shipped), dispatched the identical `drag_handle()` real-mouse helper that succeeded on the primary direction, waited 5 seconds for a new entry in the `use-case-edge-*` testid set, and timed out. No edge appeared. Validator source (`src/viewpoints/useCase/isValidConnection.ts:25-32`) reads correct — it accepts `Actor → UseCase` per #519's deferral-closing change. The fault therefore lies somewhere between source-handle mousedown and the use-case viewpoint's edge-creation handler, not in the typing layer that walk-32 and walk-33 chased. **#548** captures three candidate hypotheses for iter-890+ engineer investigation: (1) store-level duplicate-edge dedupe rejecting the secondary because the primary already authored an edge between the same Actor↔UseCase pair (testable by clean-start repro — drag actor-first, before any preceding primary); (2) `connectionMode` on the use-case ReactFlow instance set to `Strict` and rejecting the start-handle pairing for a reason not yet diagnosed; (3) a viewpoint-level `onConnectStart` filter by `data-use-case-node-kind` excluding actor-as-source. Convergence semantics applied per A.12 #3: walk surfaces a fresh `type:bug`, chain resets to 0 — not "holds at 1." Walk-33 had taken chain to 0/3 by filing #528; walk-34 was the chain[1] retry candidate; surfacing #548 reset chain back to 0. Walk-35 becomes the new chain[1] candidate against the next `vphase-15.N` bundle containing the #548 fix. The dim-10 (Use Case SysML conformance) score-3 promotion that walk-34 was staged to enable also held — score-3 requires the relationship set including association to work cleanly, and "association in one drag direction only" is not the SysML convention. Rubric stays at 3 × score-3 (dim 5 BDD, dim 6 IBD, dim 14 Round-trip integrity). The structural lesson this iteration consolidates: **the typing layer and the wiring layer are independently necessary**. PR #531 closed the typing portion (handle source-typing was the literal cause of #528's "no handle to drag from" finding), but the wiring path through ReactFlow's connection machinery has a second gap that the typing fix does not address. Walk-33's surface evidence (no handle found) was true and the fix for it shipped correctly; walk-34's deeper evidence (handle now present but drag still no-ops) is a finer-grained finding that needed the typing fix to land before it could even be detected. The architect's broad-sweep regression instrument did what it was built to do — catch the *next* gap once the previous gap was closed, rather than declaring victory on the first green light. **A.12 termination state after iter-889 close:** A.12 #1 holds at 3 × score-3 (dim 10 promotion deferred to post-#548 walk-35); A.12 #2 fails again (#548 is open `type:bug` `p1`); A.12 #3 at chain[0 / 3] (walk-35 = new chain[1] candidate post-`vphase-15.11` / `v1.6.2`); A.12 #4 still blocked behind dim-10 path.

**Links:**
- Issue filed in iter-888 execute: https://github.com/michaeljfazio/mbse-workbench/issues/548
- Iter-888 walk-34 execute PR (committed walk-34.md sections + STATUS sync): https://github.com/michaeljfazio/mbse-workbench/pull/550
- Iter-887 walk-34 plan-seal PR: https://github.com/michaeljfazio/mbse-workbench/pull/547
- Iter-886 vphase-15.10 / v1.6.1 release JOURNAL entry: https://github.com/michaeljfazio/mbse-workbench/blob/main/JOURNAL.md
- Walk-34 file (full execution log + acceptance table): https://github.com/michaeljfazio/mbse-workbench/blob/main/docs/architect/walks/walk-34.md
- PR #531 (iter-879 fix that walk-34 found incomplete): https://github.com/michaeljfazio/mbse-workbench/pull/531
- ADR 0017 (tag-cutting cadence): https://github.com/michaeljfazio/mbse-workbench/blob/main/docs/adr/0017-tag-cutting-cadence.md
- Quality rubric: https://github.com/michaeljfazio/mbse-workbench/blob/main/docs/architect/quality-rubric.md
...
## Iteration 895 — 2026-05-19 — Walk-36 surfaces dim-17 backlog; chain resets 1 → 0

**Event:** escalation

**Phase:** phase:15 — Architect-driven UX & feature hardening

**Narrative:** Walk-36 was the dim-17 deep-dive — five edge-editing affordances (reconnect endpoints, add/remove waypoints, per-edge routing-style picker, label drag, per-edge stroke/style picker under the STRICT rubric reading) tested across BDD + Activity + IBD against the byte-identical `vphase-15.10` / `v1.6.1` Pages bundle. Iter-894's plan-seal had already committed the prediction: a pre-plan-seal code scan of `CanvasPane.tsx` + `src/viewpoints/*/` confirmed all five sub-criteria were un-wired (`reconnectable` not set, no waypoint API, routing hard-coded per edge kind, label `pointerEvents: 'none'`, no per-edge style picker), so the walk's job was measurement — turning a hidden shadow score into explicit file-able findings — not surprise. The driver ran in 8.2s of headed Chromium against the deployed bundle, exercising the affordances on a freshly authored BDD composition and an Activity control flow (IBD ConnectionUsage was deferred per § Scope; the workbench-wide affordance findings generalise to IBD without separate evidence). Outcome landed exactly on row 3 of walk-36.md § Acceptance / rubric impact: 2 PASS (PC7 zero-errors cross-cutting + PC8 persistence cross-cutting) / 18 FAIL (BDD + IBD + Activity × 6 affordances). 100% pre-plan-seal prediction accuracy; zero surprises. The honest deep-dive costs a chain reset — convergence chain (A.12 #3) goes from 1 / 3 back to 0 / 3, with the same precedent set at iter-889 when walk-34 reset for an unrelated reason. Per A.10's score-honesty rule and A.16's "cosmetic-only-fixes" risk-register entry ("rubric scored 3 on visuals but app still unusable"), this trade is correct: the load-bearing deliverable is the engineer-batch backlog, not the chain advance. Five issues filed under A.7's one-issue-per-defect (with cross-viewpoint workbench-wide misses captured as `area:cross-cutting` rather than per-viewpoint duplicates): #562 (PC1 reconnect, p1) leads the priority queue because it is the cheapest score-3 unlock (React Flow v12 ships `reconnectable` + `onReconnect` natively, no metamodel change), #563 bundles PC2 + PC3 waypoint editing (one feature, two interactions — paired by UX convention, shared metamodel surface), #564 / #566 share the Inspector-edge-style infrastructure (routing-style picker + stroke/color picker), and #565 is the label-drag affordance with per-edge offset persistence. The rubric row for dim 17 now carries PC-level evidence from this walk and names the five issues as explicit promote pre-requisites — replacing the prior "walk-29 confirms PC3" line, which was true but only addressed one of six score-3 sub-criteria. The structural lesson this iteration consolidates is **a rubric dimension cannot promote on score-2 evidence alone, no matter how reinforced it is — score-3 requires per-sub-criterion evidence, and the absence of *that* evidence is itself a finding worth surfacing**. Dim 17 sat at 2 with PC3 reinforced four walks running; walk-36 turned the unreinforced sub-criteria into concrete acceptance-criteria-bearing issues so the next dim-17 promotion attempt has a tractable definition of done. Iter-896 begins the engineer-hat sequence with #562 as the first batch (smallest diff, highest ROI), with #564 + #566 grouped as the second (shared Inspector plumbing) and #565 + #563 as the third (shared edge-data extension); A.8's soft cap of 5 in-flight branches permits 2-3 of these to run in parallel as the dispatch decides. Termination state at iter-895 close: A.12 #1 unchanged at 4 × score-3 + 21 × score-2 + 3 × score-0; A.12 #2 goes from satisfied to un-satisfied with 5 open `phase:15 type:feature` issues (#562–#566), which engineer batches will close; A.12 #3 reset to 0 / 3; A.12 #4 still gated on the prior two.

**Links:**
- Walk-36 file (full plan + execution + decide-next + close-out): https://github.com/michaeljfazio/mbse-workbench/blob/main/docs/architect/walks/walk-36.md
- Iter-894 walk-36 plan-seal PR: https://github.com/michaeljfazio/mbse-workbench/pull/561
- Iter-895 walk-36 execute close-out PR (this iteration): pending
- Five filed issues: #562, #563, #564, #565, #566
- Dim-17 conventions research (visual-standards.md § 2026-05-19): https://github.com/michaeljfazio/mbse-workbench/blob/main/docs/architect/visual-standards.md
- Quality rubric (dim 17 evidence updated): https://github.com/michaeljfazio/mbse-workbench/blob/main/docs/architect/quality-rubric.md
- Iter-889 walk-34 escalation precedent (chain reset 1 → 0): https://github.com/michaeljfazio/mbse-workbench/blob/main/JOURNAL.md
- Live app (`vphase-15.10` / `v1.6.1` deploy — walk-36 target): https://michaeljfazio.github.io/mbse-workbench/
- AGENT.md A.7 (issue filing) / A.8 (PR batching) / A.10 (rubric saturation) / A.12 (termination conditions) / A.14 (JOURNAL triggers) / A.16 (risk register): https://github.com/michaeljfazio/mbse-workbench/blob/main/AGENT.md
