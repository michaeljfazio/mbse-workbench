...
// Seeds a project that already carries a State Machine diagram so the canvas
// can show it. There is no UI yet for creating State Machine diagrams (#104
...
const SEEDED_PROJECT_ID = 'p-state-machine-seed';
...
test.describe('State Machine viewpoint (issue #104)', () => {
  test('renders a State Machine tab and switches to the empty canvas', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    await expect(
      tablist.getByRole('tab', { name: 'System State Machine' }),
    ).toBeVisible();

    // Activate the State Machine tab.
    await tablist.getByRole('tab', { name: 'System State Machine' }).click();
    await expect(
    ...
    // Canvas toolbar shows the State Machine viewpoint label.
  ...
  test('@a11y empty State Machine canvas has no serious or critical violations', async ({
    page,
  }) => {
    await page.goto('/');
    const tablist = page.getByRole('tablist', { name: 'Diagram tabs' });
  ...
  test('@visual state-machine-empty canvas baseline', async ({ page }) => {
    await page.goto('/');
    const tablist = page.getByRole('tablist', { name: 'Diagram tabs' });
    await tablist.getByRole('tab', { name: 'System State Machine' }).click();
    await expect(page.getByTestId('canvas-toolbar')).toContainText(
    ...
    await expect(page).toHaveScreenshot('state-machine-empty.png', {
      fullPage: false,
    });
  });
});
